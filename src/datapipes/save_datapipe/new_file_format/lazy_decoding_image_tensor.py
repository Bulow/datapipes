...
#%%
class LazyDecodingImageTensor:
    def __init__(self, frames, lengths, offsets, decode_func: Callable[[Iterable[bytes]], np.ndarray|torch.Tensor], individual_frame_shape: Optional[torch.Size]=None):
        self.frames = frames
        self.lengths = lengths
        self.offsets = offsets

        self.decode_func: Callable[[Iterable[bytes]], np.ndarray|torch.Tensor] = decode_func
    ...
    def __getitem__(self, index) -> torch.Tensor:
        ...
        out: np.ndarray|torch.Tensor = self.decode_range(index)
    ...
    def decode_range(self, index: int|slice) -> np.ndarray|torch.Tensor:
        ...
        return self.decode_func(frame_stream_views)
