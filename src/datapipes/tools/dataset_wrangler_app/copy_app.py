...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('copy_app.log'),
...
manager = ParallelProcessManager(run_task=copy_file_chunked, verify_task=verify_hashes)


def plan_and_start(selected, dest_dir: str, workers: int) -> Tuple[Dict[str, Any], str]:
    try:
        logger.info(f"Starting copy operation with {workers} workers")
        ...
        for src in selected_paths:
            ...
            dst = str(dest_dir_path / src_path.name)
...
def build_app() -> gr.Blocks:
    try:
        ...
        with gr.Blocks(title="Parallel File Copier") as demo:
            copy_ui()
...
def copy_ui(default_input_dir: Path=Path(os.getcwd()), default_output_dir: Path=Path(os.getcwd())):
    # gr.Markdown(
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            output_folder, output_drive_stats = folder_picker(default_folder=default_output_dir, label="Output folder")
            ...
            connect_drive_stats_bar(output_folder, output_drive_stats, file_explorer=file_explorer, ignore_selection_if_path_on_same_device_as=input_folder, selection_size_action="add_selection_size")
            ...
            workers = gr.Slider(1, 16, value=4, step=1, label="Max parallel file moves")
            start_btn = gr.Button("Start copying", variant="primary", elem_id="run_btn")
    ...
    def disable_button():
        return gr.Button("Copying...", interactive=False), gr.Button(interactive=True)

    def enable_button():
        return gr.Button("Start copying", interactive=True), gr.Button(interactive=False)
    ...
    start_btn.click(
        ...
        fn=plan_and_start,
        inputs=[file_explorer, output_folder, workers],
...
if __name__ == "__main__":
    try:
        logger.info("Starting copy app")
        app = build_app()
        app.launch()
    except Exception as e:
        logger.critical(f"Critical error starting app: {e}", exc_info=True)
        raise
